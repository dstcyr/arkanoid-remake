...
void Debris::Update(float dt, const Rect<float>& bounds, const Grid& grid, const Paddle& paddle)
{
    m_anim.Update(dt);
    ...
    else if (movementPhase == 1)
    {
        ...
        if ((grid.CheckCollision(pX, pY, m_transform.w, m_transform.h, &id)) || (pX <= LEFT_WALL_X || pX >= RIGHT_WALL_X - m_transform.w) || (pY <= TOP_WALL_Y || pY >= BOTTOM_WALL_Y))
        {
            pY = m_transform.y;
            canMoveDown = false;
        }
    ...
    else if (movementPhase == 2)
    {
        ...
        if ((grid.CheckCollision(pX, m_transform.y, m_transform.w, m_transform.h, &id)) || (pX <= LEFT_WALL_X || pX >= RIGHT_WALL_X - m_transform.w) || (pY <= TOP_WALL_Y || pY >= BOTTOM_WALL_Y))
        {
            pX = m_transform.x;
        }
...
void Debris::SetPosition(float x, float y)
{
    m_transform.x = x;
    m_transform.y = y;
}

void Debris::GetTransform(Rect<float>* transform)
{
    // TODO (REVIEW) : add a gameobject for moving objects ?
